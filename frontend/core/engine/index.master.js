...
export function diff2(newNode, oldNode) {  //@
  if(!oldNode) {
...
export function patch2(parent, patches, index = 0) { //@
  if (!patches) { return }
  ...
  switch (patches.type) {
    ...
    case REPLACE: {
      const {newNode} = patches; console.log(el)
      const newEl = newNode.render ? newNode.render() : createElement(newNode);
